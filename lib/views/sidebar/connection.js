...
Connection.prototype.oncontextmenu = function oncontextmenu(e, conn) {
  ...
  , click: function() {
      e.preventDefault()
      this.target.showSettings(conn.name)
      return false
    }
  }))
...
Connection.prototype.render = function render(conn) {
  ...
  return h('.connection', [
    h('ul.nav.nav-pills.nav-stacked', [
      h('li.pure-menu-item', [
        h('a.pure-menu-link', {
          href: id
        , id: id
    ...
    , h('ul.channels.nav.nav-pills.nav-stacked', chans)
    , h('li.menu-heading.messages-heading', 'Messages')
    , h('ul.messages.nav.nav-pills.nav-stacked', privs)
