...
Handler.prototype.topic = function topic(msg) {
  ...
  if (active instanceof Connection) {
    conn = active
    if (!msg.channel) {
      debug('no channel supplied %j', msg)
      return
    }
  } else if (active instanceof Channel) {
    conn = active._connection
    if (!msg.channel) {
      msg.channel = active.name
    }
  }

  if (!conn) {
    debug('invalid connection to topic', msg)
    return
  }

  debug('setting topic for channel %s %s', msg.channel, msg.topic)
  conn.irc.topic(msg.channel, msg.topic)
}
