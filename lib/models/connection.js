...
Connection.prototype.list = function list(m) {
  if (m) {
    this.write(`LIST ${m}`)
  } else {
    this.write('LIST')
  ...
  let count = 0
  let win = null
  this.irc.once('RPL_LISTSTART', () => {
    debug('show list window')
    //show new list window
    win = this.showListWindow()
    win.toggleDevTools()
    win.on('closed', () => {
      win = null
    })
  ...
  const listener = (msg) => {
    ...
    data.push({
      name: params[1]
    , count: +params[2]
    , desc: msg.trailing ? linker(msg.trailing) : ''
    })

    if (count % 100 === 0) {
      win && win.webContents.send('list', data)
      data = []
    }
  }

  this.irc.on('RPL_LIST', listener)

  this.irc.once('RPL_LISTEND', () => {
    debug('list ended')
    if (data.length) {
      win && win.webContents.send('list', data)
      data = []
    }
    win && win.webContents.send('list:end')
    this.irc.removeListener('RPL_LIST', listener)
  })
}

Connection.prototype.showListWindow = function showListWindow() {
  const remote = require('electron').remote
  const Window = remote.require('browser-window')
  const win = new Window()
  const res = process.env.EYEARESEE_RESOURCE_PATH
  const fp = `file://${res}/views/list.html`
  win.loadURL(fp)
...
Connection.prototype._setupLogging = function _setupLogging(cb) {
  ...
  if (!this.settings.get('logTranscripts')
    || !this.settings.get('logLocation')) {
...
Connection.prototype.write = function write(msg) {
  debug('write %s', msg)
  this.irc.client.write(msg)
}
...
Connection.prototype._addIRCListeners = function _addIRCListeners() {
  ...
  this.irc.on('mode', (msg) => {
    debug('mode', msg)
    const name = (msg.target || '').toLowerCase()
    const channel = this.channels.get(name)
    if (!channel) {
      debug('invalid target', msg)
      return
    }
    ...
    const cl = msg.client
      ...
      : ''
    channel.addMessage({
      message: `${msg.nick} sets mode ${msg.mode}${cl}`
    , type: 'info'
    , to: null
    , from: null
    , hostmask: null
    , mention: msg.client === this.user.nickname
    })

    this.write(`NAMES ${channel.name} ${msg.client}`)
  })
  ...
  this.irc.on('invite', (msg) => {
    require('../handlers/invite')(msg, this)
  })
  ...
  this.irc.on('channel_mode', (msg) => {
    ...
    if (chan) {
      debug('add channel mode %s', mode)
      chan.setMode(mode)
    } else {
      debug('got channel_mode with no channel', msg)
    }
  })
  ...
  this.irc.on('topic_who_time', (msg) => {
    ...
    const chan = this.channels.get(channel)
    if (chan) {
      chan.setTopicChanged(msg.nickname, msg.date.toGMTString())
    }
  })
...
Connection.prototype._join = function(name, key) {
  if (!name) {
    ...
    if (current && current.type === 'channel') {
      name = current.name
      debug('join current channel %s', name)
      this.irc.join(name, null)
    }
...
Connection.prototype._log = function _log(msg, ts) {
  if (this.settings.get('logTranscripts') && this.logger) {
    const d = new Date(ts)
    this.logger.write(`[${d.toISOString()}] ${msg}`)
  }
}

// This sucks, but it makes bookkeeping of the previous and next channels
// significantly easier
Connection.prototype.updatePanels = function updatePanels() {
  this._panels.clear()
  for (const chan of this.channels) {
    this._panels.set(chan[0], chan[1])
  }

  for (const chan of this.privateMessages) {
    this._panels.set(chan[0], chan[1])
  }
}

Connection.prototype.addChannel = function addChannel(opts, fetch) {
...
Connection.prototype._part = function _part(name, msg) {
  if (!name) {
    const current = this.app.nav.current
    debug('try to part active %s', current && current.name)
    if (current) {
      name = current.name
    }
  }
...
Connection.prototype.removeChannel = function removeChannel(name) {
  ...
  if (this._autoJoins.has(n)) {
    this._autoJoins.delete(n)
  }
