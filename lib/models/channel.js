...
function Channel(opts) {
  ...
  this.url = `${this._connection.url}/channels/${this.name}`
...
Channel.prototype.setTopic = function setTopic(topic, who) {
  if (topic !== this.topic) {
    this.topic = topic
    const msg = who
...
Channel.prototype.setTopicChanged = function setTopicChanged(by, at) {
...
Channel.prototype.render = function render() {
  if (this.isActive()) {
    this._connection.render()
  }
}

Channel.prototype.setMode = function setMode(mode) {
  if (mode !== this.mode) {
    this.mode = mode
    this.addMessage({
...
Channel.prototype.removeUser = function removeUser(name, message) {
  ...
  if (this._connection.settings.get('showEvents')) {
    ...
    const n = utils.formatNameForType(user.nickname, 'part')
    this.addMessage({
      message: `${n} ${addr} ${pMsg}${m}`
    , type: 'part'
    , to: null
    , from: null
    , hostmask: {
