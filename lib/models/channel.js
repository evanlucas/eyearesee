...
Channel.prototype._setupLogging = function _setupLogging(cb) {
  ...
  if (!conn.logTranscripts || !conn.logLocation) {
    if (this.logger) {
...
Channel.prototype._logFile = function _logFile() {
  ...
  const t = this.type === 'channel'
    ? 'Channels'
    : 'Messages'
  return path.join(conn.logLocation, 'Connections', conn.name, t, this.name)
}
...
Channel.prototype._log = function _log(msg, ts) {
  const conn = this._connection
  if (conn.logTranscripts && this.logger) {
    const d = new Date(ts)
...
Channel.prototype.removeUser = function removeUser(name) {
  ...
  if (this._connection.showEvents) {
    // add a message
    const user = this.users.get(name)
    this.addMessage({
      message: `${user.nickname} (${user.username}@${user.address}) ${pMsg}`
    , type: 'part'
    , to: null
    , from: null
    , hostmask: {
