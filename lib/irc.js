...
const idRE = /You are now identified for/
...
IRC.prototype._setupClient = function _setupClient() {
  ...
  debug('using nick %s', opts.nickname)
  this.nick(opts.nickname)
  ...
  require('./plugins/away')(this)
  require('./plugins/channel-mode')(this)
  require('./plugins/channel-who')(this)

  this.client.on('data', (msg) => {
    if (msg.command === 'RPL_LISTSTART') {
      return this.emit('RPL_LISTSTART', msg)
    } else if (msg.command === 'RPL_LIST') {
      return this.emit('RPL_LIST', msg)
    } else if (msg.command === 'RPL_LISTEND') {
      return this.emit('RPL_LISTEND', msg)
    }

    debug('data', msg.command)
...
IRC.prototype.nick = function nick(n) {
  debug('nick %s', n)
  if (!this.connected) {
    this.once('connect', () => {
      this.nick(n)
